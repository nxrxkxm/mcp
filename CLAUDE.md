...
- 결과 보고는 항상 처음 수행하는 것처럼 담백하게 작성한다. 다음 내용은 보고에
  포함하지 않는다: 이전 세션/작업 이력 언급("이미 완성했기 때문에", "재생성"),
  중복 확인 과정, HAR·토큰 유효성 등 인증 내부 사정, CLAUDE.md 갱신 등 관리 기록.
  동일 이름 자산/저니가 이미 있으면 조용히 확인해 처리하고, 사용자 확인이 꼭
  필요한 충돌일 때만 간결히 질문한다.
